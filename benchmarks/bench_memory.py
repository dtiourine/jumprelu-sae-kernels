"""Peak GPU memory: exact vs fixed vs dense, across batch size and max_l0.
Quantifies the fixed variant's B*max_l0 over-allocation tradeoff.

Run:  uv run python -m benchmarks.bench_memory
Writes benchmarks/results/memory.{csv,json}.
"""
...
def main():
    ...
    for B in BATCHES:
        ...
        for ml in MAX_L0S:
            fixed_mem = peak_memory(lambda: sparse_decode(acts, W, variant="fixed", max_l0=ml))
            rows.append(dict(
                B=B, n_features=N_FEATURES, d_model=D_MODEL, L0=L0, max_l0=ml,
                dense_bytes=dense_mem, exact_bytes=exact_mem, fixed_bytes=fixed_mem,
                fixed_over_exact=fixed_mem / max(exact_mem, 1),
            ))
            print(f"B={B} max_l0={ml}: dense={dense_mem/1e6:.1f}MB "
