...
from kernel_jumprelu_sae.fixed.kernels import (
    compute_csr_kernel,
    sparse_decode_kernel,
)


def build_csr(feature_acts: torch.Tensor, BLOCK_F: int = 1024, max_l0: int = 512):
    ...
    per-token count afterward. The kernel guards against writing past a token's
    region (no memory corruption); if any token fired more than max_l0 features
    this function raises, since the result would be incorrect. max_l0 must
...
def sparse_decode(feature_acts, W_dec, max_l0: int = 512):
    ...
    Args:
        ...
        max_l0: fixed per-token capacity; must be >= the max L0 across tokens
            (a token firing more than max_l0 features raises ValueError).
    Returns:
        [B, d_model] reconstruction, equal to feature_acts @ W_dec.
    """
    W_dec = W_dec.contiguous()
    flat_idx, flat_val, counts, B, ml0 = build_csr(feature_acts, max_l0=max_l0)
